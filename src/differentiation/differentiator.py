...
from src.differentiation import differential_superclass as evo
import numpy as np
from src import processing as pro


def train(mu, sigma, r, samp_freq, beans, iterations, layers_dims, multi=True):
    ...
    model = evo.POPULATION(iterations, mu, sigma, r, samp_freq, layers_dims, ind_mutations=False)
...
class DIFFERENTIATOR:
    def __init__(self, number_of_runs, num_iters, indivs_per_gen, sigma, rate_of_mutation,
    ...
    def do_job(self, tasks, results):
        while True:
            try:
                ...
                training_df, testing_df, neural_net = train(self.indivs_per_gen, self.sigma,
