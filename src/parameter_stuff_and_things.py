...
def init_parameters_no_indiv_mutation(dataShape, layersSize, sigma=10):
    ...
    for layer in range(1, len(layersSize)):
        parameters["W" + str(layer)] = np.random.normal(loc=0, scale=sigma,
...
def init_parameters(dataShape, layers_size, sigma=10):
    ...
    for layer in range(1, len(layers_size)):
        parameters["W" + str(layer)] = np.random.normal(loc=0, scale=sigma if (type(sigma) == float) or (
                    type(sigma) == int) else sigma[layer]
...
def update_parameters(parameters, sigma, layers_size):
    ...
    for layer in range(1, len(layers_size)):
        ...
        rand_gauss = np.random.normal(loc=0,
                                      scale=sigma if (type(sigma) == float or type(sigma) == int) else sigma[layer]
                                      , size=(layers_size[layer], layers_size[layer - 1]))
...
def forward(data, parameters, layers_size):
    ...
    print(str.format("lenThing: {0};data shape: {1};A shape: {2};layers_size: {3}", str(lenThing), str(data.shape),
                     str(A.shape), str(layers_size)))
